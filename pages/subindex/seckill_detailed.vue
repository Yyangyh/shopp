<template>
	<view class="content">
		...
		<view class=""  @click="jump('../threeLayers/comment_list?goods_id='+data.id+'&type=1')">
			<discuss :comments='comments'></discuss>
		</view>
		<view v-if="comments !=''" class="more"  @click="jump('../threeLayers/comment_list?goods_id='+data.id+'&type=1')">
			查看更多评论
		</view>
		...
		<view class="bottom_tab"  v-if="end == true">
			...
			<view class="tab_list tab_right">
				...
				<text @click="purchase()">马上抢</text>
			</view>
		</view>
		
		<view class="bottom_tab" v-else="end == false">
			<button>抢购时间结束</button>
		</view>
	...
	export default {
		...
		methods:{
			...
			purchase(){ //马上抢
				...
				let spec = []
				let index = 0
				for (let s of this.choose_list) {
					spec.push({
						type:s.name,
						value:this.norms.split('-')[index]
					})
					index ++
				}
				console.log(spec)
				let data ={
					...
					spec:spec,
					type:'goods',
					seckill_id:this.data_list.seckill_id,
				}
				uni.navigateTo({
		...
		onLoad(options) {
			...
			this.share_arr.Url = 'http://wx.huanqiutongmall.com/h5/#/pages/subindex/seckill_detailed?data='+options.data
			this.data_list = JSON.parse(options.data)
			console.log(this.data_list)
			this.norms = this.data_list.spec_name
			// console.log(norms.splice('-'))
			this.service.entire(this,'get',this.service.api_root.subindex.Detail,{goods_id:JSON.parse(options.data).id},function(self,res){
				console.log(res)
...
<style scoped>
	...
	.product_price .price .already{
		font-size: 24rpx;
	}
	.product_price .price .price_test{
		font-weight: bold;
		color: #FF431D;
		font-size: 36rpx;
	}
	.product_price .price .price_test text{
		color: #999999;
		font-size: 24rpx;
